...
void DrawDepthGC( string File , In3str1v * paraFA04 )
{
	...
	OUTR<<""
		...
		"p <- ggplot(df, aes(GC_ratio, Depth,colour=Numbers)) + geom_point(size=1.5,alpha = 1/4) +theme(legend.direction=\"horizontal\",legend.position= \"bottom\",legend.title = element_text(face = \"bold\",size=7),panel.background = element_rect(fill = \"white\", colour = \"black\")) +scale_colour_gradient(low = \"blue\");\n"
...
//int main(int argc, char *argv[])
{
	...
	if  (!(paraFA04->List).empty())
	{
		DrawDepthGC(  (paraFA04->List)[0] ,  paraFA04 ) ;
		delete paraFA04 ;
		return 1 ;
	}
