...
int  BamStatQC_help()
{
	cout <<""
		"\n"
		"\tUsage: StatQC  -InList  <bam.list>   -OutPut  <OutPrefix>\n"
		"\n"
		"\t\t-InList      <str>     Input Bam/Sam File List\n"
		"\t\t-InFile      <str>     Input Bam/Sam File [repeat]\n"
		"\n"
		"\t\t-OutDir      <str>     OutPut File Prefix[./]\n"
...
//int main(int argc, char *argv[])
{
	...
	OUTR<<""
		...
		"text(x ="<<X_max<<", y ="<<Y_max<<", col=\"red\",labels = \" Peak GC% :"<<X_max<<"\");\n"
		"dev.off();\n"

		"read.table(\""<<OutIS<<"\",header=T)->r;\n"
		"pdf(\""<<OutIS<<".pdf\");\n"
		"plot(r[,1],r[,2],col=\"blue\",type=\"l\",ylab=\"Number\",xlab=\"Insert Size\",xlim=c(0,"<<X_InMax*4<<"))\n"
		"text(x ="<<X_InMax<<", y ="<<Y_InMax<<", col=\"red\",labels = \" Peak Insert Size% :"<<X_InMax<<"\");\n"
		"dev.off();\n"

		"read.table(\""<<OutQC<<"\")->r;\n"
		"pdf(\""<<OutQC<<".base.pdf\",w=12,h=6);\n"
		"plot(r[,2],r[,3],col=\"red\",type=\"l\",ylim=c(0,50),xlab=\"Position alone reads\",ylab=\"Prercent (%)\")\n"
